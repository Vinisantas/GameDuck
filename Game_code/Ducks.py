...
class Duck:
    def __init__(self, x, y, speed_y, speed_x):
        ...
        self.direction = random.choice([-1, 1])  # Direção inicial: direita
        self.image = pg.image.load('./assets/duck.png').convert_alpha()
        self.rect = self.image.get_rect(center=(x, y))

    def move(self):
        # Movendo o pato para cima (da parte inferior para a superior da tela)
        duck_height = self.image.get_width()
        ...
        if self.x <= 0 or self.x >= W_WIDTH - duck_height:
            self.direction *= -1
